...
training_steps = 800 # reference of iccv2017  == epoch
batch_size = 42
display_step = 10

# Network Parameters
num_hidden = 18 # hidden layer num of features
num_input = 512*3 # data input
timesteps = 18 # timesteps
...
def RNN(x, weights, biases):
    ...
    lstm_cell = rnn.LSTMCell(num_hidden, forget_bias=0.9,reuse=tf.AUTO_REUSE)
...
for iter in range(test_num):
    ...
    #fusioned_features=np.load('features/ucf101_vgg_finetunned_all_fusion_2048.npy')
    fusioned_features = np.load('features/finetunned_ucf101_vgg_jpl_all_fusion_subevent_7.npy')
    ...
    with tf.Session() as sess:
        ...
        for step in range(1, training_steps+1):
            for i in range(20):
                ...
                batch_x=batch_x[i,:,:]
                batch_y=batch_y[i]
                #batch_x = batch_x.reshape((, timesteps,num_input))
        ...
        for line in test_list:
            ...
            test_data[:,:] = fusioned_features[int(video_num2)-1,:,:] #20,2048
            for i in range(num_classes):
                if (i == video_label2):
            ...
            list_test_predi.append(test_prediction)
            list_test_acc.append(test_acc)


        print("Testing Accuracy:", np.average(test_acc))
        print(list_test_predi)
