...
def main():
    ...
    for i in range(video_num):
        ...
        features_of_flow = np.load('../dataset/jpl/finetunned_featuremap/human_flow_featuremap_2_add/' + str(i + 1) + '.npy')
        ...
        #features_of_raw=np.load('../dataset/jpl/finetunned_featuremap/human_diff_featuremap_2/'+str(i+1)+'.npy')
        features_of_optical = np.load('../dataset/jpl/finetunned_featuremap/nohuman_flow_featuremap_2_add/' + str(i + 1) + '.npy')
        """
        ...
        for j in range(20):
            ...
            for z in range(512):
                ...
                #channel_raw=np.array(channel_raw)*3.5
                #fusion=np.matmul(channel_raw,channel_flow)
                fusion2 = np.average(channel_raw)+np.max(channel_raw)+np.average(channel_flow)+np.max(channel_flow)+np.average(channel_optical)+np.max(channel_optical)
                #fusion2=np.sum(channel_raw+channel_flow+channel_optical)
    ...
    np.save('features/finetunned_ucf101_vgg_jpl_all_fusion_subevent_31.npy', final_feature)
