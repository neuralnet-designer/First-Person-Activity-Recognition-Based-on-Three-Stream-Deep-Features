...
def main():
    ...
    for i in range(video_num):
        ...
        #features_of_raw = np.load('../dataset/jpl/finetunned_featuremap/human_raw_featuremap_2_2/' + str(i + 1) + '.npy')
        ...
        for j in range(20):  # 20
            ...
            for z in range(512):  # 512
                ...
                ha_score=np.sum(attention_ha)
                #print(attention_ha)
                #print(attention_scaore)
                #print('-----------------')

                """
                SVD_hm = np.linalg.svd(np.dot((frame_feature_flow[z, :, :]).T,
                                                              (frame_feature_flow[z, :, :])))
                ...
                hm_score = np.sum(attention_hm)


                SVD_cm = np.linalg.svd(np.dot((frame_feature_optical[z, :, :]).T,
                                                              (frame_feature_optical[z, :, :])))
                u_cm, s_cm, v_cm = SVD_cm
                xa_cm = np.dot(frame_feature_optical[z, :, :], u_cm[2].T)
                xb_cm = np.dot(frame_feature_optical[z, :, :], v_cm[2].T)
                attention_cm = xa_cm * xb_cm
                cm_score = np.sum(attention_cm)

                #sum_score=np.sum(attention_ha+attention_hm+attention_cm)
                """
                #temp = np.average(frame_feature_raw[z, :, :] + frame_feature_flow[z, :, :]) + \
                #                  np.average(frame_feature_flow[z, :, :] + frame_feature_optical[z, :, :]) + \
                #                   np.average(frame_feature_raw[z, :, :] + frame_feature_optical[z, :, :])

                #human_camera[j, z] = temp+ha_score+ np.max(attention_ha)


                #temp = np.average(attention_ha + frame_feature_flow[z, :, :]) + \
                #                    np.average(frame_feature_flow[z, :, :] + frame_feature_optical[z, :, :]) + \
                #                   np.max(attention_ha)   -> ALWAYS 92%!!! -> MAY BE SOMETHIS CAN BE ADDED TO INCREASE PERFORMANCE
                temp = np.average(frame_feature_optical[z, :, :]+frame_feature_flow[z, :, :])+np.max(attention_ha)
                human_camera[j, z] = temp
    ...
    np.save('features/finetunned_ucf101_vgg_jpl_all_fusion_svd_9_19.npy', final_feature)
