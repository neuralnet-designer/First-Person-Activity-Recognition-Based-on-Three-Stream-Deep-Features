...
def main():
    ...
    #final_feature = np.zeros((video_num*17, 512 * 4), dtype=float)
    final_feature_test = np.zeros(((video_num) * 17, 512 * 4), dtype=float)
    label_test=[]

    val_file = "jpl_all2.txt"
    ...
    for line in val_list:
        ...
        for j in range(20):  # 20
            ...
            for z in range(512):  # 512
                ...
                SVD_ha = np.linalg.svd(np.dot((frame_feature_raw[z, :, :]+frame_feature_optical[z, :, :]).T, frame_feature_raw[z, :, :]+frame_feature_optical[z, :, :]))
                ...
                temp=np.average(frame_feature_raw[z, :, :] + frame_feature_flow[z, :, :]) + \
                       ...
                       np.average(frame_feature_optical[z, :, :] + frame_feature_flow[z, :, :])+\
                np.max(frame_feature_raw[z, :, :] + frame_feature_flow[z, :, :]) + \
                np.max(frame_feature_raw[z, :, :] + frame_feature_optical[z, :, :]) + \
                np.max(frame_feature_optical[z, :, :] + frame_feature_flow[z, :, :])
                       # np.average(frame_feature_optical[z, :, :] + frame_feature_flow[z, :, :]+frame_feature_raw[z, :, :])

                human_camera[j, z] = np.average(frame_feature_optical[z, :, :])#temp+np.max(attention_ha)
