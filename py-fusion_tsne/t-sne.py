...
def main():
    ...
    val_file = "index/jpl/jpl_test2.txt"
    ...
    for line in val_list:
        ...
        features_of_raw = np.load('../dataset/jpl/finetunned_featuremap/human_raw_featuremap_2_2/' + str(video_num) + '.npy')
        ...
        for j in range(20):  # 20
            ...
            for z in range(512):  # 512
                ...
                SVD_ha = np.linalg.svd(np.dot((frame_feature_raw[z, :, :] + frame_feature_flow[z, :, :]).T,
                                              (frame_feature_raw[z, :, :] + frame_feature_flow[z, :, :])))
                ...
                temp2=np.average(frame_feature_raw[z, :, :] + frame_feature_flow[z, :, :])+ \
                      np.average(frame_feature_optical[z, :, :] + frame_feature_flow[z, :, :])+\
                      np.average(frame_feature_raw[z, :, :] + frame_feature_optical[z, :, :] + frame_feature_flow[z, :, :])

                human_camera[j, z] =temp2 +np.max(attention_ha)
